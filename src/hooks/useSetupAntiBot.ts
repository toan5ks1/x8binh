...
import { handleMessageAntiBot } from '../lib/listeners/anti-bot';
import {
  LoginParams,
  LoginResponse,
  LoginResponseDto,
...
export function useSetupAntiBot(bot: LoginParams) {
  const {
    state,
    gameStatus,
    game,
  ...
  useEffect(() => {
    if (lastMessage !== null && user) {
      ...
      const newMsg = handleMessageAntiBot({
        message,
        initialRoom,
        setInitialRoom,
        setCrawingRoom,
  ...
  useEffect(() => {
    if (shouldPingMaubinh) {
      ...
      const intervalId1 = setInterval(() => {
        setITime((prevITime) => {
          const newTime = prevITime + 1;
          sendMessage(`["7", "Simms", "1",${newTime}]`);
          return newTime;
        });
      }, 5000);
